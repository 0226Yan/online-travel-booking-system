<template>
  <div>
    <div style="margin-top: 10px">
      <el-input v-model="flightOrdersData.orderId" style="width: 200px; margin-left: 240px"
...
export default {
  ...
  methods: {
    load(pageNum) {
      ...
      request.get("/flightOrder/selectPage", {
        params: {
          ...
          orderId: this.flightOrdersData.orderId,
        }
      }).then(res => {
        if (res.code === '0') {
    ...
    finished(id) {
      request.get('/flightOrder/finished/' + id).then(res => {
        if(res.code === '0') {
