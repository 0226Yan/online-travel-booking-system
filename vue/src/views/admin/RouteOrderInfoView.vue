<template>
  <div>
    <div style="margin-top: 10px">
      <el-input v-model="routeOrdersData.orderId" style="width: 200px; margin-left: 240px"
...
export default {
  ...
  methods: {
    load(pageNum) {
      ...
      request.get("/routeOrder/selectPage", {
        params: {
          pageNum: this.pageNum,
          pageSize: this.pageSize,
          orderId: this.routeOrdersData.orderId,
        }
      }).then(res => {
        if (res.code === '0') {
          this.routeOrdersData = res.data?.list
    ...
    del(id) {   // 单个删除
      this.$confirm('您确定删除吗？', '确认删除', {type: "warning"}).then(response => {
        request.delete('/routeOrder/delete/' + id).then(res => {
          if (res.code === '0') {
    ...
    finished(id) {
      request.get('/routeOrder/finished/' + id).then(res => {
        if(res.code === '0') {
